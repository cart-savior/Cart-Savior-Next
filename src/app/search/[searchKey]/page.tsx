...
import { Box, Flex, Heading, Text, Link, Button, Icon, SimpleGrid, Skeleton } from '@chakra-ui/react';
  ...
  return (
    <>
      <Heading as="h2" size="md" mt={10}>
        <Text as="span" color="brand.500">{searchKey}</Text> 키워드로 검색한&nbsp;
        <Text as="span" color="brand.500">{LIST.length}개</Text>의 결과입니다.
      </Heading>
        ...
        <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={4} mt={6}>
          {data.map((info, index) => (
            <Box key={index} p={6} bgColor="white" rounded="lg" boxShadow="md">
              ...
              <Box mb={4}>
                <Text fontSize="xl" fontWeight="700" color="brand.500">1000원</Text>
              ...
              <Flex justifyContent="space-between" alignItems="center">
                <Text fontSize="sm" color="gray.500">
                  {new Date(info.searchDate).toLocaleDateString('ko-KR', { year: '2-digit', month: 'numeric', day: 'numeric' })} / 1kg 기준
                </Text>
