...
import { ItemDetail, ItemTips } from "@/types/item";
import styles from "@/app/icons.module.scss";
import axios from "axios";

type Unit = "1kg" | "500g" | "100g" | "1g";

async function getItemTips(itemName: string): Promise<ItemTips> {
...
export default function DetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const itemTips = await getItemTips(item.item_name);
      setItemTipsData(itemTips);
    };
    fetchData();
  }, []);
      ...
      {item && (
        <Box as="section" p={4}>
          ...
          <Skeleton isLoaded={!!item}>
            <Grid templateColumns="repeat(3, 1fr)" gap={4} mb={4}>
              ...
              >
                ...
                <Text fontSize="x-large" fontWeight="bold" mt={4}>
                  ...
                  {item.diff > 0 && (
                    <Text as="span" color="red.500">
                      {item.diff.toLocaleString()}원&nbsp; 더 비싸요.
                      <Icon
                        as={TrendingUp}
                        boxSize={7}
                  ...
                  {item.diff < 0 && (
                    <Text as="span" color="blue.500">
                      {item.diff.toLocaleString()}원&nbsp; 더 싸요.
                      <Icon
                        as={TrendingDown}
                        boxSize={7}
                        verticalAlign="middle"
                ...
                <Text color="gray.500">
                  지난 주 평균가 : {item.last_week.toLocaleString()}원
                </Text>
              </GridItem>
              <GridItem
                colSpan={{ base: 3, md: 2 }}
                    ...
                    <PopoverContent>
                      ...
                      <PopoverBody>
                        <VStack alignItems="flex-start" gap={2}>
                          <Text fontSize="sm" color="gray.600">
                            지난 주 : {item.last_week_date}
                          </Text>
                          <Text fontSize="sm" color="gray.600">
                            지난 달 : {item.last_month_date}
                          </Text>
                          <Text fontSize="sm" color="gray.600">
                            지난 해 : {item.last_year_date}
                          </Text>
                          <Text fontSize="sm" color="gray.600">
                            자료 : 한국농수산식품유통공사
                          </Text>
          ...
          <Heading
            ...
            size="md"
            mt={8}
          >{`좋은 ${item.item_name} 고르는 방법`}</Heading>
          ...
          <Skeleton isLoaded={!!itemTipsData}>
            <Box mt={4} p={4} rounded="lg" bgColor="white" boxShadow="md">
              ...
              <Text as="span" mr={2} fontSize="md" color="gray.600">
                {itemTipsData?.recipe.ingredients.join(", ")}
              </Text>
              <Divider mt={4} />
